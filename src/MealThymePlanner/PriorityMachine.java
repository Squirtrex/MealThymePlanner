public class PriorityMachine {
  ...
  public static double calcPriority(User user, Recipe recipe) {
    int user_rating = user.userRatings.get(recipe.getName());
    ...
    for (RecipeIngredient ri : ingredients) {
      ...
      i_score += i_prefs.get(ri.ingredient.getName());
    }
    ...
    for (RecipeTag rt : recipe_attributes) {
      ...
      a_score += a_prefs.get(rt);
    }
    
    i_score /= len(ingredients); //Average of ingredient preferences
    a_score /= len(recipe_attributes); //Average of attribute preferences
  ...
  public Recipe[] simpleRecThree(User user, int sample_size)
  {
    ...
    String[] keys = all_recipes.keys().toArray();
    ...
    for (int i=0; i<sample_size; i++) {
      ...
      random_key = key_pool[rand.nextInt(key_pool.size())];
    ...
    Recipe[] reccomendations = getTopThree(user, sampled_recipes);
